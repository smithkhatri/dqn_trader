...
It learns entirely by trial and error. It just gets rewarded or punished based on what happens to its portfolio.

The 6 building blocks
1. The Environment — the "world" the agent lives in. It feeds the agent market data, accepts its decisions, and hands back a reward.
...
6. Evaluation & Visualization — plotting performance vs. a buy-and-hold baseline so we can actually see if our agent learned something real.
...
This is how the agent updates its beliefs. If we take action A in state S, get reward R, and land in state S', then:
...
A second copy of the neural net, frozen for N steps, used only to compute the "target" Q-value in the Bellman equation. Without it, we're chasing a moving target. The network is trying to predict values produced by itself, which causes training to spiral. The target net gets synced to the main net every N steps.
